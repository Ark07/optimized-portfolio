...
var resizePizzas = function(size) {
  ...
  var containrlength = document.querySelectorAll(".randomPizzaContainer");
    for (var i = 0; i < containrlength.length; i++) {
...
function updatePositions() {
  ...
  // accessing Dom of class name mover
  var items = document.getElementsByClassName("mover");
  // outside for loop , prevents recalculation again and again
  var calc = document.body.scrollTop / 1250;
  var length = items.length;
 // stores postion of pizza on screen
  var pizzaray = [];
  for( var i =0 ; i<5 ;i++){
    pizzaray.push(Math.sin(calc + i ))
    }

    //recalculating style
  for (var i = 0; i < length ; i++) {
  //  console.log(phase )
    items[i].style.left = items[i].basicLeft + 100 * pizzaray[i%5] + 'px';
  }
